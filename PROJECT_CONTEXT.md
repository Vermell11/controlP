...
Current state: V1.5.2.1 delivered; V1.5.3 in progress. The semantic/RAG boundary remains read-only and deterministic-first. Authorized intents use a canonical port backed by SQLite transactions, unique idempotency keys and an append-only event table; legacy JSONL is imported once with corruption accounting. Exact SHA-256 preview confirmation is still required to reach `queued`. No mutation runner is enabled yet. Voice lifecycle recovers wake/PTT after TTS/errors. The model has no filesystem, write or credential access. Data contract `SCHEMA_VERSION 2`.
